...
class EavUniqueValidator extends Validator {
    ...
    public function validateAttribute($model, $attribute) {
        ...
        $targetClass = $this->getTargetClass($this->entityValModel);
        $targetAttribute = $this->targetAttribute === null ? $attribute : $this->targetAttribute;
        $rawConditions = $this->prepareConditions($targetAttribute, $model, $attribute);
        $conditions[] = $this->targetAttributeJunction === 'or' ? 'or' : 'and';
        foreach ($rawConditions as $key => $value) {
            if (is_array($value)) {
                $this->addError($this->entityValModel, $attribute, Yii::t('yii', '{attribute} is invalid.'));
                return;
            }
            $conditions[] = [$key => $value];
        }
        if ($this->modelExists($targetClass, $conditions, $this->entityValModel)) {
...
//        } else {
        ...
        if ($n === 1) {
            ...
            $exists = ($pk != $this->entityValModel->getOldPrimaryKey(true));
        } else {
